...
<!--              </select>-->
              ...
              <span v-if="classify" style="margin-left: 0px;line-height: 50px">如不修改可以不选|当前选择：<b>{{checkedClassifyName}}</b></span>
...
<script>
import {getArticle,addSort,updateSort,deleteSort} from "../../api/index"
    export default {
        name: "Up",
        components:{},
      data(){
      ...
      methods:{
        async gainArticle(id){
            ...
            //  this.info=res.data;
            this.inputs.articleContent=res.data.article.articleContent;
            this.inputs.discription=res.data.article.discription;
            this.image.path=res.data.article.articleImg;
            this.inputs.articleTitle=res.data.article.articleTitle;
            this.inputs.articleId=res.data.article.articleId;
            this.checkedClassifyId=res.data.sort.sortId;
            this.checkedClassifyName=res.data.sort.sortName;
            this.inputs.tags=res.data.labels;

            
            this.image.fid=-1;

            $("#myiframe").contents().find(".note-editable").html(this.inputs.articleContent)
        },
        initData(){
          // alert();
          this.gainArticle(this.$route.query.id);
        },
        getContent(){
